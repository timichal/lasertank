...
number in `SOLVER.md` unreproducible?** If yes, and it is a few kilobytes, it belongs here. Two
...
(`SOLVER.md`, *Next actions* item 4: the flags survived only as directory names, so neither is
reproducible).
...
- **Campaign reports** (`*.jsonl`, including `chain.jsonl`) — outputs, and they encode *what the
  ...
  *Next actions* has the recipe for rebuilding it by unioning the four chain reports. **This is the
  one exclusion the session-29 widening does not touch**, and the reason is the failure mode rather
