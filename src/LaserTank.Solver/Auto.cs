...
namespace LaserTank.Solver
{
    internal static class Auto
    {
        ...
        internal static int Run(Program.Args a)
        {
            ...
            // The batch harness defaults to ./solutions because a campaign is
            // always launched with an explicit --out anyway.  A bare
            // interactive command has none, and it should not drop a directory
            // git does not ignore into whatever the working directory is.
            string outRoot = a.OutGiven
                ? a.Out : Path.Combine(root, "build", "solutions", "auto");
            string outDir = Path.Combine(outRoot, collection);
            string work = Path.Combine(outRoot, ".auto");
