...
| 10 | last | wall clock: **instrumented in session 49, built and gated in session 50**. `--push-memo` memoises `PushH` on `(playfield, tank cell, and PF2 under --push-stop)` and is **1.67x on the arm the fourth pass runs**, 1.40x on the shipped rung, 1.42x on `layer7` — **`IDENTICAL` on all three**, reports field for field and every `.lpb` byte for byte (`bash tools/push_memo.sh`). **On by default since session 51** — `--no-push-memo` is the way off, and re-gating after the flip is `IDENTICAL` again at **1.31 / 1.56 / 1.33x** on an idle machine. What is left is only the second, board-keyed layer underneath it, worth a further **~1.12x** | done, flipped and re-gated; ~12 min to re-run the gate |
