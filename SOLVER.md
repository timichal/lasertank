...
| **4** | **2nd** | the campaign that decides whether `--best-of-round` is a **default**, and it inherits closed item 13's rule — keep a round open while `shots > ghs_shots`, now shipped as `--best-of-shots`. **The instrument is built (session 48) and what is left is machine time**: the driver takes `--report` and the selection flags, so a campaign of driver runs can be compared at last, and `bash tools/bor_campaign.sh` runs three arms over two populations and prints the table | **~6-7 h at `JOBS=4`** beside item 2's pass; `... a` is ~1.5 h of it, `... report` is free |
