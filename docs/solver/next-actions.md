...
## 2 (1st) — the fourth pass, and it now wants a fourth arm
...
|---|---|---:|---:|---|
| l8work | the layer-8 set plus `--push-eval work` | **61** (23.9%) | 5 | 61 |
| layer7 | `--push-stop 1 --push-shot-run 16 --push-beam 128` | 55 | **8** | +15 → 76 |
| enables | `--push-enables 8` | 52 | 2 | +5 → **81** |
| l8none | the layer-8 set plus `--push-eval none` | 37 (14.5%) | 1 | +2 → 83 |
| l8learned | `--push-reach --push-ferry-match --push-ferry-maze --push-dead 20 --push-fire 8 --push-shot-run 16 --push-beam 128 --max-keys 5000` | 57 | 1 | +1 → 84 |
...
Run `l8work` first — it is the largest single result, so it lands earliest if the run is interrupted.
...
### The fourth arm to rehearse: `--push-fire-tier`

**It is a much better candidate than `--push-eval none` was.** On the 138 unsolved short-record GAUNTLETs
it is **85 solo against the same arm's 76, with 15 exclusive levels against 6** — it wins solo *and* it is
complementary, which no rejected arm has ever done. `none` was 37 solo against `plain`'s 38 with one
exclusive level in six arms.

**Rehearse it before believing it, for `none`'s own reason:** those 138 are selected for the shape the
tier is for, and a filtered population over-reports complementarity as readily as strength. The rehearsal
is 2 h 15 m against ~18 h for the arm, and it paid for itself last time even though the answer was no.
...
`$L8 --push-eval work --push-fire-tier`, on the same 255-level stride the other six were run on.
...
## 10 (5th) — wall clock on the push rungs
...
## 6 (last) — the blogspot goal-board harvester

**Checked far enough to cost, not started.** It is last because it is not a solver change at all, and
nothing above it is blocked on it.
