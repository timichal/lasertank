...
| **Reversibility as a resource** — bricks shot, thin ice walked, an anti-tank killed or pushed up, a mirror that cannot turn 360° (hard pt 1 §3) | **mostly missing.** The frozen block and the hole/block count are the only irreversibility the search knows. Nothing prices *spending* a brick, a thin-ice tile or an anti-tank for no derived reason — but the distribution is measured (session 42): a change that consumes something the read has no other reason for is 7.6% of what is offered and **1.9%** of what the human does, **0.04%** with the anti-tank kill exempted, so a spend *tier* is right-signed. Item 16 closed on that; the tier itself is unbuilt, because it needs `Opens` and `Enables` per successor and the search rations both — and item 17 is now the reason to price it against a population before building it, the `rare` tier having been refused on a bench its 3.15x did not predict |
| **Phases: a saving in one must not cost another** (pt 7; hard pt 1 §2) | **built, and the answer is no on level 6.** The series' definition is an *independence* property; what shipped is a cheaper test that needs no model — a phase ends at a **milestone**, a board change after which some consumable object is strictly rarer. Level 6 duly comes apart into six phases of 18-34 board changes inside a horizon of 50, `--push-phases` commits to the right first board, and **the beam cannot reach phase 2 — not at width 128, and not from the human's own board**. So the phases are real and the search cannot walk them, which is a finding about the ranking rather than about phases. [Closed item 5](history.md#5-level-6s-decomposition--built-and-refused-by-the-falsifier-it-set-itself) |
| **"Two by two", not one by one** (pt 4, pt 1) | **heeded, and it was the right warning.** Carries are batched *because* a lone carry wastes the round trip, and level 6's hand line moves five blocks before the first one sinks. `--push-ferry-stage` bet the other way and *"fills one hole and stalls"*, so `--push-phases` lets a phase be a **group** of carries — its milestone test says nothing about how many blocks move on the way — and its first phase does exactly what the human's does. It is the *next* phase that fails, so this warning cost nothing |
...
| **The editor: build the pivotal situation and play from it** (hard pt 2 §4) | **built, and it answered.** `--push-seed FILE.lpb:K` searches from a recording's K-th board change; on level 6 the horizon is **50 of 168 board changes** and the boundary is exactly one change wide. That is item 5's falsifier and the first direct measurement of the quantity every layer since 5 attacks. Item 15 closed on the one level; item 18 ran it over all 20 and found a **25x** spread, so the quantity is per-level |
| **Work backwards from the ending position** (hard pt 2 §7, quoting Sahan) | **outlived the item that named it**: it was FESS's packing order, and [item 23 closed negative](history.md#23-the-fess-rung--the-space-separates-levels-or-moves-inside-one-and-never-both) on its feature space rather than on this. The series is still independent confirmation that the backward half is the one a Sokoban wants, and nothing measured it |
| **Thinking like an author** — *"why is that object there?"* (hard pt 2 §6) | **measured, and it is the largest lift the read has ever shown.** `--read-rare N` counts each element class on the board *as authored* and asks whether a change touches one the level has N or fewer of: 5.1% of the successors on offer, **16.0%** of what the human does, **3.15x**, against 1.3-1.5x for the three derivations the read already ships. Item 16 closed on the distribution and **the rung was built and refused** (item 17, session 44): as `--push-rare` in front of `TierAdvance` it is **ferry 18 → 16 with no exclusive level, deep 25 → 24**, and inert at `--read-rare 1`. The lift was real and belonged to *these recordings* — on the ferry bench the derivation names **0%** of successors on four levels of five, the authored census having three or more of everything a ferry push touches. [Closed item 17](history.md#17-the-rarity-tier--built-and-refused-on-the-bench-it-set-itself) |
...
Four actionable things, and they became [`next-actions.md`](next-actions.md) items 13-16. The list's own
ordering rule (*cheapest falsifier first, whatever kind of work it is*) put three of them ahead of item 5
and gave item 5 the cheap falsifier it had been open without — and item 5 has since closed on that
falsifier's descendant, negative. **All four have now closed** — three in
session 43, and item 16's surviving tier proposal (item 17) refused in session 44 — so the ordinals below
are the ones they were spent at, not the current list:
