...
| **Phases: a saving in one must not cost another** (pt 7; hard pt 1 §2) | **the definition is new, and item 5 needs it.** [Item 5](next-actions.md#5-2nd--level-6s-decomposition) is *commit to one (block, hole) pair, re-derive*, and until item 15 it was "the one open item with no cheap falsifier". Lyf defines a phase as an **independence** property, which is testable — and item 15's horizon now sizes one: **50** board changes on level 6 (item 18's bisection; item 15 read 48 from a coarser bracket) -- but item 18 also showed the horizon is a *level* property, 2 to 50 over 12 recordings, so the size is not a constant to build against |
...
and gave item 5 the cheap falsifier it had been open without. **All four have now closed** — three in
