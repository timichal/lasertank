...
src/        the C# port         build.sh -> build/lasertank-{core,solve}.exe
  ...
  LaserTank.Game/   the Godot 4.7 project
                    ...
                    CollectionList.cs  command 108's picker;  CollectionCheck.cs
                                   its headless dump
                    CollectionNotes.cs  what each collection is and which shelf
                                   it is on — laser-tank.com's facts, rewritten
...
| `options_check.py` | the importer's semantics, and the typed store's round trip — including that the `LaserTank.ini` it read comes back **byte for byte** and that the import happens exactly once; every pack loading with mode 1 == mode 2 pixel for pixel; the three board sizes; and the laser bar measured out of a `--shot` PNG **in the cell the engine names**. Since step 15 also the filtered walk: `[OPT] SkipComLev` and `[DATA] Diff_Setting` read, written and masked, and `--check-advance`'s stop sequences compared against the same walk recomputed in Python from the `.lvl` and `.hs` bytes — including a record whose `SDiff` is zeroed on purpose, because unfilterable is a case worth building rather than looking for. Since step 17 also command 118's stack, over `--check-history`: the push is one per *change* of level, a restart is not one, back past the first is refused rather than wrapped, 108 clears it and a failed 108 does not, and the stack goes deeper than the original's ten slots — the expected stack rebuilt in Python by the same model the walk uses |
| `sound_check.py` | every recorded `.lpb` through both engines with `--sound`, so the per-tick `SoundPlay` id stream must be identical too; the sixteen WAVs decoding to the same PCM in Python and C#; `[OPT] Sound`'s semantics on the import side, and the round trip through the store |
